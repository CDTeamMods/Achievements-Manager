  ...
  async finishSetup() {
    ...
    if (liteModeToggle) {
      ...
      debugManagerPromise?.then(debugManager =>
        debugManager?.log('\u{1F50B} Modo Lite capturado:', liteModeToggle.checked)
      );
    }
    if (virtualScrollToggle) {
      this.setupData.virtualScrolling = virtualScrollToggle.checked;
      debugManagerPromise?.then(debugManager =>
        debugManager?.log('\u{1F4DC} Virtual Scrolling capturado:', virtualScrollToggle.checked)
      );
    }
    if (languageSelect) {
      this.setupData.language = languageSelect.value;
      debugManagerPromise?.then(debugManager =>
        debugManager?.log('\u{1F30D} Idioma capturado:', languageSelect.value)
      );
    }
    const settings = {
      setupComplete: true,
    ...
    debugManagerPromise?.then(debugManager =>
      debugManager?.log('\u{1F4BE} Configura\xE7\xF5es finais a serem salvas:', settings)
    );
