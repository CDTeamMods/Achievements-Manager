const require_steam_local_games = module => {
      ...
      try {
        ...
        for (const steamAppsPath of this.steamPaths) {
          const files = await fs.readdir(steamAppsPath).catch(err => {
            this.debugManager?.warn(`\u26A0\uFE0F Erro ao escanear ${steamAppsPath}:`, err.message);
            return [];
          });
...
const steam_local_games_default = require_steam_local_games();
export { steam_local_games_default as default };
