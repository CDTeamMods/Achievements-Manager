var require_crash_reporter = async () => {
  ...
  class CrashReporter {
    ...
    setupIPC() {
      ipcMain.handle('crash-reporter:report-error', async errorData => {
        try {
    ...
    async reportCrash(type, error, context = {}) {
      try {
        ...
        const fileName = `crash_${crashId}_${timestamp.replaceAll(/[:.]/g, '-')}.log`;
    ...
    async reportWarning(warning) {
      try {
        ...
        const fileName = `warning_${warningId}_${timestamp.replaceAll(/[:.]/g, '-')}.log`;
        const filePath = path.join(this.crashReportsPath, fileName);
...
const crash_reporter_default = require_crash_reporter();
export { crash_reporter_default as default };
