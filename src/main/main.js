const require_main = async module => {
  ...
  const getDebugManager =
    debugModule.getDebugManager || (debugModule.default && debugModule.default.getDebugManager);
  ...
  const setupFileSystem =
    filesystemModule.setupFileSystem ||
    (filesystemModule.default && filesystemModule.default.setupFileSystem);
  const windowManagerModule = require('./modules/window-manager');
  const setupWindowManager =
    windowManagerModule.setupWindowManager ||
    (windowManagerModule.default && windowManagerModule.default.setupWindowManager);
  const performanceModule = require('./modules/performance');
  const setupPerformance =
    performanceModule.setupPerformance ||
    (performanceModule.default && performanceModule.default.setupPerformance);
  const crashModule = require('./modules/crash-reporter');
  const setupCrashReporter =
    crashModule.setupCrashReporter ||
    (crashModule.default && crashModule.default.setupCrashReporter);
  ...
  const getSecurityManager =
    securityModule.getSecurityManager ||
    (securityModule.default && securityModule.default.getSecurityManager);
  const getSandboxManager =
    sandboxModule.getSandboxManager ||
    (sandboxModule.default && sandboxModule.default.getSandboxManager);
  const gseModule = require('./modules/gse-saves');
  const GSESavesManager =
    gseModule.GSESavesManager ||
    (gseModule.default && gseModule.default.GSESavesManager) ||
    gseModule;
  ...
  const GoldbergMigrationManager =
    goldbergModule.GoldbergMigrationManager ||
    (goldbergModule.default && goldbergModule.default.GoldbergMigrationManager) ||
    goldbergModule;
  const steamIntegrationModule = require('./modules/steam-integration');
  const SteamIntegrationManager =
    steamIntegrationModule.SteamIntegrationManager ||
    (steamIntegrationModule.default && steamIntegrationModule.default.SteamIntegrationManager) ||
    steamIntegrationModule;
  const steamLocalModule = require('./modules/steam-local-games');
  const SteamLocalGamesManager =
    steamLocalModule.SteamLocalGamesManager || steamLocalModule.default || steamLocalModule;
  const pathModule = require('./modules/path-manager');
  const setupPathManager =
    pathModule.setupPathManager || (pathModule.default && pathModule.default.setupPathManager);
  const achievementsModule = require('./modules/achievements');
  const setupAchievements =
    achievementsModule.setupAchievements ||
    (achievementsModule.default && achievementsModule.default.setupAchievements);
  ...
  function createMainWindow() {
    ...
    const securityManager =
      typeof getSecurityManager === 'function'
        ? getSecurityManager()
        : (() => {
            throw new Error('getSecurityManager is not available');
          })();
    const sandboxManager =
      typeof getSandboxManager === 'function'
        ? getSandboxManager()
        : (() => {
    ...
    setupWindowEvents();
    const closeSplashIfPresent = () => {
      if (splashWindow && !splashWindow.isDestroyed()) {
        splashWindow.close();
        splashWindow = null;
      }
    };
    const boostWindowFocus = () => {
    ...
    const handleDidFailLoad = validatedURL => {
...
const main_default = require_main();
